...
describe("Contract — LLM Scaffold", function () {
  ...
  describe("isBot40(address)", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.isBot40(addr1.address /* TODO_AI */);
      // TODO_AI: expect(await contract.isBot40(addr1.address /* TODO_AI */)).to.equal(/* expected */);
    });

    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.isBot40("0x0000000000000000000000000000000000000000" /* TODO_AI: use zero/unauthorized */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
