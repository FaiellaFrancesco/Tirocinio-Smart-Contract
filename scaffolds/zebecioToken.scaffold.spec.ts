...
describe("zebecioToken — LLM Scaffold", function () {
  async function deployFixture() {
    const [owner, addr1, addr2] = await ethers.getSigners();
    const Factory = await ethers.getContractFactory("zebecioToken");
  ...
  describe("getzebecioVC()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.getzebecioVC();
      // TODO_AI: expect(await contract.getzebecioVC()).to.equal(/* expected */);
    });

    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.getzebecioVC()
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
  ...
  describe("zebecioVC()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.zebecioVC();
      // TODO_AI: expect(await contract.zebecioVC()).to.equal(/* expected */);
    });
