...
import { loadFixture } from "@nomicfoundation/hardhat-toolbox/network-helpers";
  ...
  /**
   * Scaffold automatically generated for AIAstroNet.
  ...
  describe("add_vEIaP(uint256)", function () {
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_vEIaP(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_yAsITC(uint256)", function () {
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_yAsITC(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
