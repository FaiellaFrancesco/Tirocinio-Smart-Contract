...
/**
 * Scaffold generato automaticamente per CortexNeural.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("CortexNeural — LLM Scaffold", function () {
  ...
  describe("add_DDNkn(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_DDNkn(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_eeuZPk(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_eeuZPk(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
