import { expect } from "chai";
  ...
  /**
   * Scaffold automatically generated for contracts/medium/0x27e4202bf7d1ad2b871bdf07a8444750a1dddb75.sol:ZEUS.
   * Blocks marked // TODO_AI must be completed by the LLM.
   */

  describe("contracts/medium/0x27e4202bf7d1ad2b871bdf07a8444750a1dddb75.sol:ZEUS — LLM Scaffold", function () {
    async function deployFixture() {
      const { ethers } = (await import("hardhat")).default;
      const [owner, addr1, addr2] = await ethers.getSigners();
      const Factory = await ethers.getContractFactory("ZEUS");
  ...
  describe("_maxZEUSTaxSwap()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract._maxZEUSTaxSwap();
      // TODO_AI: expect(await contract._maxZEUSTaxSwap()).to.equal(/* atteso */);
    });
  ...
  describe("rescueZEUSTOKNETH()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // state-changing transaction
      const result = await contract.rescueZEUSTOKNETH();
      // TODO_AI: verifica stato/eventi dopo la tx
    });
