import { expect } from "chai";
  ...
  /**
   * Scaffold automatically generated for contracts/medium/0x49a382f956dd9aa323f26418324bbb54fa3435d6.sol:Erc20.
   * Blocks marked // TODO_AI must be completed by the LLM.
   */

  describe("contracts/medium/0x49a382f956dd9aa323f26418324bbb54fa3435d6.sol:Erc20 — LLM Scaffold", function () {
  ...
  describe("launch(address)", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // state-changing transaction
      const result = await contract.launch(addr1.address /* TODO_AI */);
      // TODO_AI: verifica stato/eventi dopo la tx
    });
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.launch("0x0000000000000000000000000000000000000000" /* TODO_AI: use zero/unauthorized */)
      ).to.be.revertedWith(/* TODO_AI: inserire messaggio */);
    });
  ...
  describe("launched()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.launched();
      // TODO_AI: expect(await contract.launched()).to.equal(/* atteso */);
    });
