...
/**
 * Scaffold generato automaticamente per BookofMattFurie.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("BookofMattFurie — LLM Scaffold", function () {
  ...
  // Eventi in ABI: Approval, OwnershipTransferred, Transfer, TransferTaxUpdated
  ...
  describe("transferTo(address,uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.transferTo("0x0000000000000000000000000000000000000000" /* TODO_AI: usa zero/non autorizzato */, 0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
