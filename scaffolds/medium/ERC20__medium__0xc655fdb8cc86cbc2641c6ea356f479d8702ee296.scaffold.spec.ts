import { expect } from "chai";
  ...
  /**
   * Scaffold automatically generated for contracts/medium/0xc655fdb8cc86cbc2641c6ea356f479d8702ee296.sol:ERC20.
   * Blocks marked // TODO_AI must be completed by the LLM.
   */

  describe("contracts/medium/0xc655fdb8cc86cbc2641c6ea356f479d8702ee296.sol:ERC20 — LLM Scaffold", function () {
  ...
  describe("revokeAllowance(address)", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // state-changing transaction
      const result = await contract.revokeAllowance(addr1.address /* TODO_AI */);
      // TODO_AI: verifica stato/eventi dopo la tx
    });
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.revokeAllowance("0x0000000000000000000000000000000000000000" /* TODO_AI: use zero/unauthorized */)
      ).to.be.revertedWith(/* TODO_AI: inserire messaggio */);
    });
