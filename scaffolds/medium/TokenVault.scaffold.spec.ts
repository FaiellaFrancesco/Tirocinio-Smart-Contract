...
import { loadFixture } from "@nomicfoundation/hardhat-toolbox/network-helpers";
  ...
  /**
   * Scaffold automatically generated for TokenVault.
   * Blocks marked // TODO_AI must be completed by the LLM.
   */

  describe("TokenVault — LLM Scaffold", function () {
    ...
    // Events in ABI: Deposited, Withdrawn
  ...
  describe("deposit(uint256)", function () {
    it("happy path", async function () {
      ...
      const result = await contract.deposit(1n /* TODO_AI */);
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.deposit(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
