...
import { loadFixture } from "@nomicfoundation/hardhat-toolbox/network-helpers";
  ...
  /**
   * Scaffold automatically generated for SolmaxToken.
  ...
  describe("excludeWalletFromFees(address)", function () {
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.excludeWalletFromFees("0x0000000000000000000000000000000000000000" /* TODO_AI: usa zero/non autorizzato */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
  ...
  describe("tokenBurn(uint256)", function () {
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.tokenBurn(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
  ...
  describe("updateFeeAmount(uint256,uint256)", function () {
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.updateFeeAmount(0n /* TODO_AI: rendi invalido/edge */, 0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
