...
/**
 * Scaffold generato automaticamente per CopiumLabs.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("CopiumLabs — LLM Scaffold", function () {
  ...
  describe("add_haDnm(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_haDnm(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_kdervd(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_kdervd(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
