...
import { loadFixture } from "@nomicfoundation/hardhat-toolbox/network-helpers";
  ...
  /**
   * Scaffold automatically generated for AIDynamics.
  ...
  describe("add_owcfe(uint256)", function () {
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_owcfe(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_ePsJyZ(uint256)", function () {
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_ePsJyZ(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
