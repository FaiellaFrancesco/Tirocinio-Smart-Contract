...
import { loadFixture } from "@nomicfoundation/hardhat-toolbox/network-helpers";
  ...
  describe("isFree_AnyERC20Tokens(address,uint256)", function () {
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.isFree_AnyERC20Tokens("0x0000000000000000000000000000000000000000" /* TODO_AI: usa zero/non autorizzato */, 0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
  ...
  describe("removeFromBlackListwallets(address[])", function () {
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.removeFromBlackListwallets([] /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
