...
/**
 * Scaffold generato automaticamente per FomoLords.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("FomoLords — LLM Scaffold", function () {
  ...
  describe("add_nCNpv(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_nCNpv(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_COuMMj(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_COuMMj(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
