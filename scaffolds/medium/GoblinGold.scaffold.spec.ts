...
/**
 * Scaffold generato automaticamente per GoblinGold.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("GoblinGold — LLM Scaffold", function () {
  ...
  describe("add_KrvKG(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_KrvKG(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_zuUApr(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_zuUApr(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
