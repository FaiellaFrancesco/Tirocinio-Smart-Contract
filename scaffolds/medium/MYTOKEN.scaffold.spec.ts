...
/**
 * Scaffold generato automaticamente per MYTOKEN.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("MYTOKEN — LLM Scaffold", function () {
  ...
  // Eventi in ABI: Approval, Burn, Transfer
  ...
  describe("burnedSupply()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // chiamata di sola lettura
      const result = await contract.burnedSupply();
      // TODO_AI: expect(await contract.burnedSupply()).to.equal(/* atteso */);
    });
  ...
  describe("burnedTokens()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // chiamata di sola lettura
      const result = await contract.burnedTokens();
      // TODO_AI: expect(await contract.burnedTokens()).to.equal(/* atteso */);
    });

    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.burnedTokens()
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
