import { expect } from "chai";
  ...
  /**
   * Scaffold automatically generated for contracts/medium/0xdf37bf81529d240bb6f6174cc7e328d36ca6e912.sol:WORMS.
   * Blocks marked // TODO_AI must be completed by the LLM.
   */

  describe("contracts/medium/0xdf37bf81529d240bb6f6174cc7e328d36ca6e912.sol:WORMS — LLM Scaffold", function () {
    async function deployFixture() {
      const { ethers } = (await import("hardhat")).default;
      const [owner, addr1, addr2] = await ethers.getSigners();
      const Factory = await ethers.getContractFactory("WORMS");
  ...
  describe("_maxWORMSTaxSwap()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract._maxWORMSTaxSwap();
      // TODO_AI: expect(await contract._maxWORMSTaxSwap()).to.equal(/* atteso */);
    });
  ...
  describe("rescueWORMSTOKNETH()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // state-changing transaction
      const result = await contract.rescueWORMSTOKNETH();
      // TODO_AI: verifica stato/eventi dopo la tx
    });
