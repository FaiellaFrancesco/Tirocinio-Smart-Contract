import { expect } from "chai";
  ...
  /**
   * Scaffold automatically generated for contracts/medium/0x16f21ca05ef8396473ba8e7e24483fa80c60d5d6.sol:OGGYTOKEN.
   * Blocks marked // TODO_AI must be completed by the LLM.
   */

  describe("contracts/medium/0x16f21ca05ef8396473ba8e7e24483fa80c60d5d6.sol:OGGYTOKEN — LLM Scaffold", function () {
    async function deployFixture() {
      const { ethers } = (await import("hardhat")).default;
      const [owner, addr1, addr2] = await ethers.getSigners();
      const Factory = await ethers.getContractFactory("OGGYTOKEN");
  ...
  describe("_maxOGGYTaxSwap()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract._maxOGGYTaxSwap();
      // TODO_AI: expect(await contract._maxOGGYTaxSwap()).to.equal(/* atteso */);
    });
  ...
  describe("rescueOGGYTOKNETH()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // state-changing transaction
      const result = await contract.rescueOGGYTOKNETH();
      // TODO_AI: verifica stato/eventi dopo la tx
    });
