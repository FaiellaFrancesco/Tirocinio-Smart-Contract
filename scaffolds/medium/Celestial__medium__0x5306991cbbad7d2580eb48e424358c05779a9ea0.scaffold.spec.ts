...
/**
 * Scaffold generato automaticamente per Celestial.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("Celestial — LLM Scaffold", function () {
  ...
  describe("add_ueDzG(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_ueDzG(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_LUHELR(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_LUHELR(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
