...
describe("JUSTICECAT — LLM Scaffold", function () {
  ...
  describe("_bots(address)", function () {
    it("happy path", async function () {
      ...
      // TODO_AI: expect(await contract._bots(addr1.address /* TODO_AI */)).to.equal(/* expected */);
    });

    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract._bots("0x0000000000000000000000000000000000000000" /* TODO_AI: use zero/unauthorized */)
