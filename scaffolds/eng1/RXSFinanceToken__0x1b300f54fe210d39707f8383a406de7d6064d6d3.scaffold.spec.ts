...
describe("RXSFinanceToken — LLM Scaffold", function () {
  async function deployFixture() {
    const [owner, addr1, addr2] = await ethers.getSigners();
    const Factory = await ethers.getContractFactory("RXSFinanceToken");
  ...
  describe("RXSFinanceVC()", function () {
    it("happy path", async function () {
      ...
      // read-only call
      const result = await contract.RXSFinanceVC();
      // TODO_AI: expect(await contract.RXSFinanceVC()).to.equal(/* expected */);
    });

    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.RXSFinanceVC()
  ...
  describe("getRXSFinanceVC()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.getRXSFinanceVC();
      // TODO_AI: expect(await contract.getRXSFinanceVC()).to.equal(/* expected */);
    });

    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.getRXSFinanceVC()
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
