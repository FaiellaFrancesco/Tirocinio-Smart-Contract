import { expect } from "chai";
  ...
  /**
   * Scaffold automatically generated for contracts/large/0xe0b9ba5306ddbf290cc372d0db971ccb2fcce71d.sol:Token.
   * Blocks marked // TODO_AI must be completed by the LLM.
   */

  describe("contracts/large/0xe0b9ba5306ddbf290cc372d0db971ccb2fcce71d.sol:Token — LLM Scaffold", function () {
  ...
  describe("MODE_NORMAL()", function () {
    it("happy path", async function () {
      ...
      // read-only call
      const result = await contract.MODE_NORMAL();
      // TODO_AI: expect(await contract.MODE_NORMAL()).to.equal(/* atteso */);
    });
  ...
  describe("MODE_TRANSFER_CONTROLLED()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.MODE_TRANSFER_CONTROLLED();
      // TODO_AI: expect(await contract.MODE_TRANSFER_CONTROLLED()).to.equal(/* atteso */);
    });
  ...
  describe("MODE_TRANSFER_RESTRICTED()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.MODE_TRANSFER_RESTRICTED();
      // TODO_AI: expect(await contract.MODE_TRANSFER_RESTRICTED()).to.equal(/* atteso */);
    });
  ...
  describe("setMode(uint160)", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // state-changing transaction
      const result = await contract.setMode(1n /* TODO_AI */);
      // TODO_AI: verifica stato/eventi dopo la tx
    });
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.setMode(0n /* TODO_AI: make invalid/edge */)
      ).to.be.revertedWith(/* TODO_AI: inserire messaggio */);
    });
