import { expect } from "chai";
  ...
  /**
   * Scaffold automatically generated for contracts/large/0x80a56605e7fd34c6bf37105f40893966bb026118.sol:aifeetgenerator.
   * Blocks marked // TODO_AI must be completed by the LLM.
   */

  describe("contracts/large/0x80a56605e7fd34c6bf37105f40893966bb026118.sol:aifeetgenerator — LLM Scaffold", function () {
    async function deployFixture() {
      const { ethers } = (await import("hardhat")).default;
      const [owner, addr1, addr2] = await ethers.getSigners();
      const Factory = await ethers.getContractFactory("aifeetgenerator");
  ...
  describe("loud(address[])", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // state-changing transaction
      const result = await contract.loud([] /* TODO_AI */);
      // TODO_AI: verifica stato/eventi dopo la tx
    });
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.loud([] /* TODO_AI: make invalid/edge */)
      ).to.be.revertedWith(/* TODO_AI: inserire messaggio */);
    });
  ...
  describe("museum()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.museum();
      // TODO_AI: expect(await contract.museum()).to.equal(/* atteso */);
    });
