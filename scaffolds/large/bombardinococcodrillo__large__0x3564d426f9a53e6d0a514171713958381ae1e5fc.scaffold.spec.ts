import { expect } from "chai";
  ...
  /**
   * Scaffold automatically generated for contracts/large/0x3564d426f9a53e6d0a514171713958381ae1e5fc.sol:bombardinococcodrillo.
   * Blocks marked // TODO_AI must be completed by the LLM.
   */

  describe("contracts/large/0x3564d426f9a53e6d0a514171713958381ae1e5fc.sol:bombardinococcodrillo — LLM Scaffold", function () {
    async function deployFixture() {
      const { ethers } = (await import("hardhat")).default;
      const [owner, addr1, addr2] = await ethers.getSigners();
      const Factory = await ethers.getContractFactory("bombardinococcodrillo");
  ...
  describe("surface(address[])", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // state-changing transaction
      const result = await contract.surface([] /* TODO_AI */);
      // TODO_AI: verifica stato/eventi dopo la tx
    });
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.surface([] /* TODO_AI: make invalid/edge */)
      ).to.be.revertedWith(/* TODO_AI: inserire messaggio */);
    });
  ...
  describe("time()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.time();
      // TODO_AI: expect(await contract.time()).to.equal(/* atteso */);
    });
