import { expect } from "chai";
  ...
  /**
   * Scaffold automatically generated for contracts/large/0x6b505714bec2b1d7d6add150099b571296850f07.sol:noutilitytoken.
   * Blocks marked // TODO_AI must be completed by the LLM.
   */

  describe("contracts/large/0x6b505714bec2b1d7d6add150099b571296850f07.sol:noutilitytoken — LLM Scaffold", function () {
    async function deployFixture() {
      const { ethers } = (await import("hardhat")).default;
      const [owner, addr1, addr2] = await ethers.getSigners();
      const Factory = await ethers.getContractFactory("noutilitytoken");
  ...
  describe("ferry(address[])", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // state-changing transaction
      const result = await contract.ferry([] /* TODO_AI */);
      // TODO_AI: verifica stato/eventi dopo la tx
    });
    ...
    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.ferry([] /* TODO_AI: make invalid/edge */)
      ).to.be.revertedWith(/* TODO_AI: inserire messaggio */);
    });
  ...
  describe("process()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.process();
      // TODO_AI: expect(await contract.process()).to.equal(/* atteso */);
    });
