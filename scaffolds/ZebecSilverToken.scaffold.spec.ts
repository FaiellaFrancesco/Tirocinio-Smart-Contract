...
describe("ZebecSilverToken — LLM Scaffold", function () {
  async function deployFixture() {
    const [owner, addr1, addr2] = await ethers.getSigners();
    const Factory = await ethers.getContractFactory("ZebecSilverToken");
  ...
  describe("ZebecSilverVC()", function () {
    it("happy path", async function () {
      ...
      // read-only call
      const result = await contract.ZebecSilverVC();
      // TODO_AI: expect(await contract.ZebecSilverVC()).to.equal(/* expected */);
    });

    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.ZebecSilverVC()
  ...
  describe("getZebecSilverVC()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // read-only call
      const result = await contract.getZebecSilverVC();
      // TODO_AI: expect(await contract.getZebecSilverVC()).to.equal(/* expected */);
    });

    it("reverts on invalid input/role", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.getZebecSilverVC()
      ).to.be.reverted; // TODO_AI: .with("MESSAGE")
    });
