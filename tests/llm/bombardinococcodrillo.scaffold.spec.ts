...
/**
 * Scaffold generato automaticamente per bombardinococcodrillo.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("bombardinococcodrillo — LLM Scaffold", function () {
  ...
  describe("surface(address[])", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.surface([] /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
