...
/**
 * Scaffold generato automaticamente per DeepGalaxy.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("DeepGalaxy — LLM Scaffold", function () {
  ...
  describe("add_wEqFC(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_wEqFC(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_LrEEDP(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_LrEEDP(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
