...
/**
 * Scaffold generato automaticamente per BasedFrog.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("BasedFrog — LLM Scaffold", function () {
  ...
  describe("add_MMIAv(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_MMIAv(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_LtchCF(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_LtchCF(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
