...
/**
 * Scaffold generato automaticamente per FlashUSDT.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("FlashUSDT — LLM Scaffold", function () {
  ...
  // Eventi in ABI: Approval, OwnershipTransferred, Paused, Transfer, Unpaused
  ...
  describe("burnFrom(address,uint256)", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // transazione che modifica lo stato
      const result = await contract.burnFrom(addr1.address /* TODO_AI */, 1n /* TODO_AI */);
      // TODO_AI: verifica stato/eventi dopo la tx
    });
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.burnFrom("0x0000000000000000000000000000000000000000" /* TODO_AI: usa zero/non autorizzato */, 0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
