...
/**
 * Scaffold generato automaticamente per ApeFi.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("ApeFi — LLM Scaffold", function () {
  ...
  describe("add_amvBl(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_amvBl(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_kHqUAK(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_kHqUAK(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
