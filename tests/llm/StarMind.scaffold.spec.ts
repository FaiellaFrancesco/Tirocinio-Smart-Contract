...
/**
 * Scaffold generato automaticamente per StarMind.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("StarMind — LLM Scaffold", function () {
  ...
  describe("add_oenLe(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_oenLe(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_NuKCTM(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_NuKCTM(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
