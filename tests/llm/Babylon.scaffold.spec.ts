...
/**
 * Scaffold generato automaticamente per Babylon.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("Babylon — LLM Scaffold", function () {
  ...
  describe("bulktransfer(address[],uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.bulktransfer([] /* TODO_AI: rendi invalido/edge */, 0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
