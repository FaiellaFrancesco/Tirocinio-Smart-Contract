...
/**
 * Scaffold generato automaticamente per BioCognition.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("BioCognition — LLM Scaffold", function () {
  ...
  describe("add_ZmSyL(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_ZmSyL(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_Nmildy(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_Nmildy(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
