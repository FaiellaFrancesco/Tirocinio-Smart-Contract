...
/**
 * Scaffold generato automaticamente per RoboBrain.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("RoboBrain — LLM Scaffold", function () {
  ...
  describe("add_LMsUw(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_LMsUw(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_tFUyhI(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_tFUyhI(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
