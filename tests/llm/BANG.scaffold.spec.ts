...
describe("BANG — LLM Scaffold", function () {
  ...
  describe("removeStuckERC20(address)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.removeStuckERC20(/* TODO_AI bad */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
  ...
  describe("setStructure(uint256,uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.setStructure(/* TODO_AI bad */, /* TODO_AI bad */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
  ...
  describe("setSwapFees(uint256,uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.setSwapFees(/* TODO_AI bad */, /* TODO_AI bad */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
  ...
  describe("updateFeeThreshold(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.updateFeeThreshold(/* TODO_AI bad */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
