...
/**
 * Scaffold generato automaticamente per WenMoon.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("WenMoon — LLM Scaffold", function () {
  ...
  describe("add_vALYB(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_vALYB(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_alNpwK(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_alNpwK(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
