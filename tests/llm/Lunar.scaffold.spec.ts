...
/**
 * Scaffold generato automaticamente per Lunar.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("Lunar — LLM Scaffold", function () {
  ...
  describe("add_IxoDz(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_IxoDz(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_qNhPyw(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_qNhPyw(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
