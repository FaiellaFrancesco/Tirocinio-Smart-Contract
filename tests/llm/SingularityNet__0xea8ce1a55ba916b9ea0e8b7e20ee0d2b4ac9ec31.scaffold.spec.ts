...
/**
 * Scaffold generato automaticamente per SingularityNet.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("SingularityNet — LLM Scaffold", function () {
  ...
  describe("add_iiWUG(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_iiWUG(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_vhHjcq(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_vhHjcq(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
