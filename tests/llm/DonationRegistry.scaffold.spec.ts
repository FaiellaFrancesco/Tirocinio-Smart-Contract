...
/**
 * Scaffold generato automaticamente per DonationRegistry.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("DonationRegistry — LLM Scaffold", function () {
  ...
  // Eventi in ABI: DonationReceived, FundsWithdrawn
  ...
  describe("donate()", function () {
    it("happy path", async function () {
      ...
      const result = await contract.donate({ value: 1n /* TODO_AI in wei */ });
  ...
  describe("getDonationTotal(address)", function () {
    it("happy path", async function () {
      ...
      const result = await contract.getDonationTotal(addr1.address /* TODO_AI */);
      // TODO_AI: expect(await contract.getDonationTotal(addr1.address /* TODO_AI */)).to.equal(/* atteso */);
    });

    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.getDonationTotal("0x0000000000000000000000000000000000000000" /* TODO_AI: usa zero/non autorizzato */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
