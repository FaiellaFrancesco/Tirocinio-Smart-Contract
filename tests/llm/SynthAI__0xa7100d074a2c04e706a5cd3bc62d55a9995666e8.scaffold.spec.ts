...
/**
 * Scaffold generato automaticamente per SynthAI.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("SynthAI — LLM Scaffold", function () {
  ...
  describe("add_YGDYL(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_YGDYL(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_snAoPK(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_snAoPK(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
