...
/**
 * Scaffold generato automaticamente per RugProof.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("RugProof — LLM Scaffold", function () {
  ...
  describe("add_UJwqs(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_UJwqs(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_rzjNgQ(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_rzjNgQ(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
