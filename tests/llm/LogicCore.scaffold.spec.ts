...
/**
 * Scaffold generato automaticamente per LogicCore.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("LogicCore — LLM Scaffold", function () {
  ...
  describe("add_IYirp(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_IYirp(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_nNdJmD(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_nNdJmD(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
