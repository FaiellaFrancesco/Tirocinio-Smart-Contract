...
/**
 * Scaffold generato automaticamente per Nexus.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("Nexus — LLM Scaffold", function () {
  ...
  describe("add_srkek(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_srkek(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_ogaXjN(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_ogaXjN(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
