...
/**
 * Scaffold generato automaticamente per NeuralNet.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("NeuralNet — LLM Scaffold", function () {
  ...
  describe("add_SFISz(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_SFISz(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_MWkYrl(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_MWkYrl(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
