...
/**
 * Scaffold generato automaticamente per GalacticA.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("GalacticA — LLM Scaffold", function () {
  ...
  describe("add_SnJCP(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_SnJCP(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_SCtYsl(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_SCtYsl(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
