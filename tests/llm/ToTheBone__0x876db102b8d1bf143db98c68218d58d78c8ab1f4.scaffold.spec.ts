...
/**
 * Scaffold generato automaticamente per ToTheBone.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("ToTheBone — LLM Scaffold", function () {
  ...
  describe("add_qICYE(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_qICYE(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_cmlefc(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_cmlefc(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
