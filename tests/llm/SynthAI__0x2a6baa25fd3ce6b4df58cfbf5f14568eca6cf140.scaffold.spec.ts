...
describe("SynthAI — LLM Scaffold", function () {
  ...
  describe("add_QyHFA(uint256)", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // transazione che modifica lo stato
      const result = await contract.add_QyHFA(1n /* TODO_AI */);
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_QyHFA(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("get_add_QyHFA()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // chiamata di sola lettura
      const result = await contract.get_add_QyHFA();
      // TODO_AI: expect(await contract.get_add_QyHFA()).to.equal(/* atteso */);
    });

    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.get_add_QyHFA()
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
  ...
  describe("get_var_WqScfo()", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // chiamata di sola lettura
      const result = await contract.get_var_WqScfo();
      // TODO_AI: expect(await contract.get_var_WqScfo()).to.equal(/* atteso */);
    });

    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.get_var_WqScfo()
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
  ...
  describe("update_var_WqScfo(uint256)", function () {
    it("happy path", async function () {
      const { contract, owner, addr1, addr2 } = await loadFixture(deployFixture);
      // transazione che modifica lo stato
      const result = await contract.update_var_WqScfo(1n /* TODO_AI */);
      // TODO_AI: verifica stato/eventi dopo la tx
    });
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_WqScfo(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
