...
describe("NeuronFlux — LLM Scaffold", function () {
  ...
  describe("add_RpkMC(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_RpkMC(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_kOQkxd(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_kOQkxd(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
