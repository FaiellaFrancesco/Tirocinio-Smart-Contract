...
describe("Contract — LLM Scaffold", function () {
  ...
  describe("isBot48(address)", function () {
    it("happy path", async function () {
      ...
      // TODO_AI: expect(await contract.isBot48(addr1.address /* TODO_AI */)).to.equal(/* atteso */);
    });

    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.isBot48("0x0000000000000000000000000000000000000000" /* TODO_AI: usa zero/non autorizzato */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
