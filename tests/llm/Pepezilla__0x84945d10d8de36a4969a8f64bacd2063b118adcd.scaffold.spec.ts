...
describe("Pepezilla — LLM Scaffold", function () {
  ...
  describe("add_CutbG(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_CutbG(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_hgLXTf(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_hgLXTf(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
