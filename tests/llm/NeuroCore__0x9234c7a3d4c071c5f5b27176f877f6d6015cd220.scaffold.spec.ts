...
/**
 * Scaffold generato automaticamente per NeuroCore.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("NeuroCore — LLM Scaffold", function () {
  ...
  describe("add_DWpRX(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_DWpRX(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_ybYAWJ(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_ybYAWJ(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
