...
/**
 * Scaffold generato automaticamente per SmartAI.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("SmartAI — LLM Scaffold", function () {
  ...
  describe("add_MvjIl(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_MvjIl(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_QvaAnp(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_QvaAnp(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
