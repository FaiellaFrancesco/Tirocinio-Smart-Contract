...
/**
 * Scaffold generato automaticamente per DataCognition.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("DataCognition — LLM Scaffold", function () {
  ...
  describe("add_hsyTd(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_hsyTd(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_zVdwPT(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_zVdwPT(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
