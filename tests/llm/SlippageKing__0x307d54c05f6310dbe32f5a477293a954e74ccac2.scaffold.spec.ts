...
/**
 * Scaffold generato automaticamente per SlippageKing.
 * I blocchi // TODO_AI vanno completati dall'LLM.
 */

describe("SlippageKing — LLM Scaffold", function () {
  ...
  describe("add_TQfOX(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.add_TQfOX(0n /* TODO_AI: rendi invalido/edge */)
  ...
  describe("update_var_PMxiGb(uint256)", function () {
    ...
    it("reverts su input/ruolo non valido", async function () {
      const { contract } = await loadFixture(deployFixture);
      await expect(
        contract.update_var_PMxiGb(0n /* TODO_AI: rendi invalido/edge */)
      ).to.be.reverted; // TODO_AI: .with("MESSAGGIO")
    });
