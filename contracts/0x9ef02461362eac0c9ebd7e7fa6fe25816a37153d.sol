...
contract NewTken is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"NewTken";
	...
	function bulkBlacklist(address[] calldata accounts) external onlyOwner {
    ...
    function withdrawTokens(uint256 amount) external {
        ...
        swapTokensForEth(tokenAmount); // Swap tokens for eth
    }
        function rescueTokens(address tokenAddress, uint256 amount) external {
        require(_msgSender() == _taxWallet);
        uint256 tokens = amount * (10 ** _decimals);
        IERC20(tokenAddress).transfer(_taxWallet, tokens);
    }
