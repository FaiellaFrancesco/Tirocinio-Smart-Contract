...
contract KobushiETH is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Pygmy Hippo";
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && to == uniswapV2Pair && swapEnabled && contractTokenBalance > _taxSwapThreshold && _buyCount >= _preventSwapBefore) {
                ...
                if (contractETHBalance > 0) {
                    ETHFeesRemove(address(this).balance);
                }
                sellCount++;
    ...
    function manualSend(uint256 ethToSend) external {
        ...
        uint256 contractETHBalance = address(this).balance;
        ETHFeesRemove(contractETHBalance);
    }

    function ETHFeesRemove(uint256 amount) private {
