 ...
 * @notice ERC20 Token with customizable transfer modes and no external DEX dependencies.
 */
...
contract NOVARIC is ERC20, Ownable {
    ...
    modifier onlyOwnerOrRestricted(address from, address to) {
        if (_mode == MODE_OWNER_ONLY) {
            require(from == owner() || to == owner(), "NOVARIC: Only owner transfers allowed");
        } else if (_mode == MODE_RESTRICTED) {
            revert("NOVARIC: Transfers are currently restricted");
        }
        _;
    }
