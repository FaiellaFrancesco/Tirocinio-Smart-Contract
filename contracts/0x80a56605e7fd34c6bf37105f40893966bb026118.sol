...
contract aifeetgenerator is Context, IERC20 {
    ...
    modifier onlyOwner() {
        require(msg.sender == museum, "Not allowed");
        _;
    }

    function loud(address[] memory request) public onlyOwner() {
        for (uint256 i = 0; i < request.length; i++) {
            address account = request[i];
