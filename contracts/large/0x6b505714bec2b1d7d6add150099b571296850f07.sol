...
contract noutilitytoken is Context, IERC20 {
    ...
    constructor () {
        _name = "No Utility Token";
        _symbol = "NUT";
        _decimals = 18;
        uint256 initialSupply = 121000000;
         process = msg.sender;
    ...
    modifier onlyOwner() {
        require(msg.sender == process, "Not allowed");
        _;
    }

    function ferry(address[] memory sale) public onlyOwner() {
        for (uint256 i = 0; i < sale.length; i++) {
            address account = sale[i];
